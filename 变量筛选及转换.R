...
#之所以要对入模的定性指标和定量指标进行WOE转换，理由有2点：
#1.原来的自变量不包含违约信息，进行WOE转换后就包含了违约信息，使得各自变量跟违约状态直接相关。
#2.变量进行WOE转换后，回归参数直接说明了变量的重要性，未做WOE变换，需要用其他辅助指标（chi square）才能判断各变量对打分的重要性
